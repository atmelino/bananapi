 ...
 * @homepage http://xdsoft.net/jqplugins/dialog/ (c) 2015, Chupurnov Valeriy
 *           <chupurnov@gmail.com>
 */
/* global jQuery,window,document,clearInterval,setInterval */
(function($) {
	'use strict';
	var plugin_name = 'dialog', tpl_button = '<button class="xdsoft_btn" type="button"></button>', tpl_dialog = '<div class="xdsoft_dialog xdsoft_dialog_shadow_effect">'
			...
			+ '</div>'
			+ '<div class="xdsoft_dialog_buttons">' + '</div>' + '</div>',
			...
			for (title in buttons) {
				if (buttons.hasOwnProperty(title)) {
					if (buttons[title] instanceof $) {
						...
						btn
								.click(createCallbackHandler(
										$.isFunction(buttons[title]) ? buttons[title]
												: ((buttons[title].click && $
														.isFunction(buttons[title].click)) ? buttons[title].click
														: false), dialog_box));
						...
						if ($.isPlainObject(buttons[title])) {
							...
							if (buttons[title].primary) {
								buttons_box.find('button').removeClass(
										'xdsoft_primary');
								btn.addClass('xdsoft_primary');
							}
	...
	$.fn[plugin_name] = function(_options, second, third) {
		var that = this, dialog_box = that, options = $.extend(true, {},
				default_options, $.isPlainObject(_options) ? _options : {}), dialog = dialog_box
				.find('.xdsoft_dialog'), buttons_box = dialog_box
				.find('.xdsoft_dialog_buttons'), event;
					...
					case 'ok':
						if (dialog_box.is(':visible')) {
							if (second !== 'enter'
								...
								buttons_box.find('.xdsoft_primary').trigger(
										'click');
							}
							if (second === 'enter'
									&& options.clickDefaultButtonOnEnter
							...
							if (second !== 'esc' || options.closeOnEsc) {
								dialog_box.stop()[options.closeFunction]
										(function() {
											...
											dialog_box.trigger('after'
													+ _options + '.xdsoft');
											if (options['onAfter' + _options]
													&& $
															.isFunction(options['onAfter'
																	+ _options])) {
											...
											if (dialog_box
													.data('resize_interval')) {
												clearInterval(dialog_box
														.data('resize_interval'));
											}
										});
								if (second === 'esc' && options.closeOnEsc
					...
					case 'show':
						if (!dialog_box.is(':visible')) {
							...
							dialog_box.stop()[options.showFunction](function() {
								dialog_box.trigger('after' + _options
										+ '.xdsoft');
								if (options['onAfter' + _options]
										&& $.isFunction(options['onAfter'
												+ _options])) {
									options['onAfter' + _options].call(that,
											options, _options);
								}
							});
							if (!options.modal) {
						...
						break;
					case 'title':
						makeTitle(second, dialog_box
								.find('.xdsoft_dialog_popup_title'),
								dialog_box, options);
						break;
					case 'content':
						dialog_box.find('.xdsoft_dialog_content').html(second);
						break;
					}
					if (_options.toLowerCase() !== 'hide'
							&& _options.toLowerCase() !== 'show') {
						dialog_box.trigger('after' + _options + '.xdsoft');
						if (options['onAfter' + _options]
								&& $.isFunction(options['onAfter' + _options])) {
							options['onAfter' + _options].call(that, options,
									_options);
						}
					}
				}
			} else {
				makeTitle(options.title, dialog_box
						.find('.xdsoft_dialog_popup_title'), dialog_box,
						options);
				makeButtons(options.buttons, tpl_button, dialog_box
						.find('.xdsoft_dialog_buttons'), dialog_box);
			}
		...
		dialog_box.on('mousedown.xdsoft', function(event) {
			if (options.closeOnClickOutside) {
				if (!hasScroll(this)
						|| event.pageX < $(document.body).width()
								- getScrollbarWidth()) {
					dialog_box[plugin_name]('hide');
				}
			}
		...
		dialog_box.dialog('content', that.length ? that[0] : '<div>'
				+ that.selector + '</div>');
		...
		makeTitle(options.title, dialog.find('.xdsoft_dialog_popup_title'),
				dialog_box, options);
	...
	window.jPromptDatabase = function(hostname, username, password, database,
			default_value, callback, title) {
		...
		// global $password;
		// global $database;
		+ 'hostname: ' + '<input  autofocus type="text"  id="hostname" value="' + hostname
				+ '"/>' + '<br>' + 'username: '
				...
				({
					...
					buttons : {
						'Ok' : function(event) {
							...
							var newhostname = document
									.getElementById('hostname').value;
							var newusername = document
									.getElementById('username').value;
							var newpassword = document
									.getElementById('password').value;
							var newdatabase = document
									.getElementById('database').value;
							(callback || function() {
							}).call(this, event, newhostname, newusername,
									newpassword, newdatabase);
						},
						'Cancel' : true
					}
				});
	...
	window.jPrompt = function(msg, default_value, callback, title) {
		...
		var tpl = '<div>'
				...
				+ (default_value ? default_value.replace(/[&<>"']/g, function(
						match) {
